...
func main() {
	...
	// rendertarget
	tw, th := 1024, 1024
	targetTexture, frameBuffer := GenerateRendertarget(tw, th)

	planeMesh := GeneratePlane(5, 5*float32(height)/float32(width))

	// vao
	planeVertexArrayObject := gl.GenVertexArray()
	defer planeVertexArrayObject.Delete()
	planeVertexArrayObject.Bind()

	// vbo's
	planeVertexBuffer := gl.GenBuffer()
	defer planeVertexBuffer.Delete()
	planeVertexBuffer.Bind(gl.ARRAY_BUFFER)
	size = len(planeMesh.Positions) * 3 * int(glh.Sizeof(gl.FLOAT))
	gl.BufferData(gl.ARRAY_BUFFER, size, planeMesh.Positions, gl.STATIC_DRAW)

	planeUvBuffer := gl.GenBuffer()
	defer planeUvBuffer.Delete()
	planeUvBuffer.Bind(gl.ARRAY_BUFFER)
	size = len(planeMesh.UVs) * 2 * int(glh.Sizeof(gl.FLOAT))
	gl.BufferData(gl.ARRAY_BUFFER, size, planeMesh.UVs, gl.STATIC_DRAW)

	planeNormalBuffer := gl.GenBuffer()
	defer planeNormalBuffer.Delete()
	planeNormalBuffer.Bind(gl.ARRAY_BUFFER)
	size = len(planeMesh.Normals) * 3 * int(glh.Sizeof(gl.FLOAT))
	gl.BufferData(gl.ARRAY_BUFFER, size, planeMesh.Normals, gl.STATIC_DRAW)

	// ebo
	planeElementBuffer := gl.GenBuffer()
	defer planeElementBuffer.Delete()
	planeElementBuffer.Bind(gl.ELEMENT_ARRAY_BUFFER)
	size = len(planeMesh.Indices) * int(glh.Sizeof(gl.UNSIGNED_SHORT))
	gl.BufferData(gl.ELEMENT_ARRAY_BUFFER, size, planeMesh.Indices, gl.STATIC_DRAW)
	...
	for ok := true; ok; ok = (window.GetKey(glfw.KeyEscape) != glfw.Press && !window.ShouldClose()) {
		...
		viewMatrix := mgl32.LookAtV(mgl32.Vec3{0, 5, 10}, mgl32.Vec3{0, 0, 0}, mgl32.Vec3{0, 1, 0})
		...
		func() {
			...
			// update uniforms
			projectionUniform.UniformMatrix4fv(false, projectionMatrix)
			viewUniform.UniformMatrix4fv(false, targetViewMatrix)

			diffuseUniform.Uniform3f(1, 1, 1)
			opacityUniform.Uniform1f(1.0)

			modelUniform.UniformMatrix4fv(false, mgl32.HomogRotate3D(angle, (mgl32.Vec3{0, 1, 0}).Normalize()))

			// bind texture
			gl.ActiveTexture(gl.TEXTURE0 + gl.GLenum(textureSlots))
			targetTexture.Bind(gl.TEXTURE_2D)
			defer targetTexture.Unbind(gl.TEXTURE_2D)
			diffuseMapUniform.Uniform1i(textureSlots)
			textureSlots++

			// bind attributes
			planeVertexArrayObject.Bind()
			defer planeVertexArrayObject.Unbind()

			planeVertexBuffer.Bind(gl.ARRAY_BUFFER)
			defer planeVertexBuffer.Unbind(gl.ARRAY_BUFFER)
			positionAttribute.EnableArray()
			defer positionAttribute.DisableArray()
			positionAttribute.AttribPointer(3, gl.FLOAT, false, 0, nil)

			planeNormalBuffer.Bind(gl.ARRAY_BUFFER)
			defer planeNormalBuffer.Unbind(gl.ARRAY_BUFFER)
			normalAttribute.EnableArray()
			defer normalAttribute.DisableArray()
			normalAttribute.AttribPointer(3, gl.FLOAT, false, 0, nil)

			planeUvBuffer.Bind(gl.ARRAY_BUFFER)
			defer planeUvBuffer.Unbind(gl.ARRAY_BUFFER)
			uvAttribute.EnableArray()
			defer uvAttribute.DisableArray()
			uvAttribute.AttribPointer(2, gl.FLOAT, false, 0, nil)
			...
			// draw elements
			planeElementBuffer.Bind(gl.ELEMENT_ARRAY_BUFFER)
			defer planeElementBuffer.Unbind(gl.ELEMENT_ARRAY_BUFFER)
			gl.DrawElements(gl.TRIANGLES, len(planeMesh.Indices), gl.UNSIGNED_SHORT, nil)
...
func GenerateRendertarget(w, h int) (gl.Texture, gl.Framebuffer) {
	...
	gl.DrawBuffers(1, []gl.GLenum{gl.COLOR_ATTACHMENT0 + gl.GLenum(0)})
	...
	return textureBuffer, frameBuffer
}
